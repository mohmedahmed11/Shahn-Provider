...
class AttachmentHandler: NSObject{
    ...
    func showAttachmentActionSheet(vc: UIViewController, withDocument: Bool = true, withImages: Bool = true) {
        currentVC = vc
...
extension AttachmentHandler: UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    func imagePickerController(didFinishPickingMediaWithInfo info: [String : Any]) {
        if let image = info[UIImagePickerController.InfoKey.originalImage.rawValue] as? UIImage {
            ...
            if let asset = info[UIImagePickerController.InfoKey.phAsset.rawValue] as? PHAsset, let fileName = asset.value(forKey: "filename") as? String {
                imageName = fileName
            } else if let url = info[UIImagePickerController.InfoKey.imageURL.rawValue] as? URL {
                imageName = url.lastPathComponent
            }
            self.imagePickedBlock?(image, imageName, "\("fileUrl")")
